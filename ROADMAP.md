...
- [ ] **Pre-existing intermittent segfault in the full suite (Linux/offscreen Qt)** — `tests/test_gr3_chart_chrome.py` hard-crashes pytest on ~30-50% of full-suite runs but passes 12/12 in isolation; confirmed present on a stashed baseline, so not introduced by recent work. No CI impact (no workflow runs pytest). Needs root-causing.
