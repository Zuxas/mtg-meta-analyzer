...
- [ ] **Replay-viewer M3: board state panel** — battlefield/hand/GY/exile thumbnails synced to event cursor via card_image_cache. Spec: same as M2.
- [ ] **Replay-viewer M4: polish** — event search, kind filter chips, jump-to-key-events, per-replay notes, markdown export, remove classic dialog after Full has been default for 1 week.
