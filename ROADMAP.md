...
- [~] **M1 data-quality fix (gates board zone-counts + tap/counters/auras)** — game_num oscillation **DONE** (2026-06-16, `feat/replay-match-scoping`, commit d8963c4): real root cause was `build_event_stream` never scoping to `arena_match_id` (every replay held all matches from both logs); now gated to the target match's region + `SCHEMA_VERSION` 1→2 to rebuild stale caches. **Still open:** sparse hand/lib/GY/exile zone tracking in `board_diff` (battlefield correct; other zones read ~0). See NEXT_STEPS.md. Foundational for the M-future Odds Engine.
