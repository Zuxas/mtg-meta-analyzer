...
- [x] **Intermittent full-suite segfault — RESOLVED as a symptom of the above** (2026-09-22) — aborted MainWindow construction orphaned `EventHubTab`'s running worker, whose queued lambda fired into freed memory on a later `processEvents()`. 2 consecutive clean runs post-fix at commit time, further runs in progress (was crashing ~30-50%).
