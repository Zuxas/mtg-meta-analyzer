...
Last updated: 2026-05-25 (M4 of full-depth replay viewer shipped: review annotation. Editable per-replay **notes** persisted to new `match_log.replay_notes` column (JSON `{text,marks}` keyed by arena_match_id; `db.match_log.get_replay_notes`/`save_replay_notes`; stub-row created if match not in log). **★ Mark-important** event toggle (marks feed the Jump-To menu). **Markdown export** of a replay review (`gui.replay_view_model.replay_markdown` + "Export review" button). Built subagent-driven over 6 TDD tasks on `feat/replay-viewer-m4`; 15 new tests, **300/300 green**. Plan: docs/superpowers/plans/2026-05-25-replay-viewer-m4.md. M3 (2026-05-25): board panel (replay_board_at + ReplayBoardPanel; zone counts hidden pending M1 fix; tap/counters/auras/combat deferred). M2 (2026-05-24): viewer window. M1 (2026-05-22): data layer. NEXT: (a) M1 data-quality fix — game_num oscillation + sparse zone tracking, unblocks board counts + tap/counters + Odds Engine; or (b) retire classic dialog ~2026-06-01.)
