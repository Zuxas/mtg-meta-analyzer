...
Last updated: 2026-05-24 (M2 of full-depth replay viewer shipped: `gui/widgets/replay_viewer_window.py` QMainWindow + Qt-free `gui/replay_view_model.py` view-model. Left timeline tree + lazy QAbstractTableModel event table + kind-filter chips + search proxy + right detail tabs (Event Details / Stack / read-only Notes) + card preview + Jump-To menu + nav buttons. Match History "▶ Watch" became a split button: Watch (Full) / Watch (Classic), last-used mode persisted in `tabs.match_history.replay_viewer_mode`. Board panel = M3 placeholder; speed/Animate = M5 placeholders. Built subagent-driven over 14 TDD tasks on branch `feat/replay-viewer-m2`; 35 new tests, 270/270 green. Plan: docs/superpowers/plans/2026-05-23-replay-viewer-m2.md. M1 (2026-05-22): data layer — analysis/replay_events.py events[]/match_meta/capabilities. Next: M3 board state panel.)
