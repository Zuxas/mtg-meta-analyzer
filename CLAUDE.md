...
Last updated: 2026-05-25 (M3 of full-depth replay viewer shipped: board state panel. `analysis.replay_events.replay_board_at(events, seq)` reconstructs per-game zones from `board_diff` (resets on game_num change; never stored). `gui/widgets/replay_board_panel.py::ReplayBoardPanel` = two-row board (life/mana + Hand/Lib/GY/Exile counts + battlefield thumbnails via card_image_cache + current-card highlight + Show-Board-Changes), driven from `_select_seq`. New widget-agnostic `gui/widgets/card_tooltip.install_card_hover` for hover-full-image on any widget. Built subagent-driven over 5 TDD tasks on branch `feat/replay-viewer-m3`; 14 new tests, **285/285 green**. DEFERRED (not in M1 data contract): tap rotation / +1/+1 counters / auras / combat highlight (opp combat absent) / lands-creatures split — need an extractor extension. Plan: docs/superpowers/plans/2026-05-24-replay-viewer-m3.md. M2 (2026-05-24): viewer window (replay_viewer_window.py + replay_view_model.py). M1 (2026-05-22): data layer (events[]/match_meta/capabilities). Next: M4 polish (event search, jump-to items, per-replay notes + match_log.replay_notes migration, markdown export, retire classic dialog).)
