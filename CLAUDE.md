...
Last updated: 2026-06-09 (Replay-viewer **M1 data-quality fix** on `feat/replay-viewer-m1-dq`. Root cause was MIS-diagnosed in prior notes: not dual-log duplication, but that `analysis/replay_events.py::build_event_stream` **never scoped GSM/client-message processing to the requested match** — it ingested every match in both log files (measured 4005 GSMs / 14 mulligans vs the real 980 / 4). Three fixes: (1) **match-scoping** — `active_room_match` tracked from `matchGameRoomStateChangedEvent` + a guard skipping non-target blobs; (2) **idempotent dedup** — GSMs by `(current_game, gameStateId)` (gsid resets per game), client msgs by `transactionId`, only when the key is non-None (handles true resends + rotation overlap); (3) **diff-aware zones** — run the disappeared-instance sweep only for zones the message actually reported (`objectInstanceIds` key present), and attribute hidden-zone cards (library, opp hand) via the zone's `ownerSeatId` (`setdefault`, so battlefield control from gameObjects wins). Also `SCHEMA_VERSION` 1→2 + schema-version gate on cache load; **serve-stale** on rebuild-miss (return the old cache, not None, for matches rotated out of the logs); `capabilities.log_offsets` corrected True→False (never populated). Verified on live logs: game_num `[1,2,3]` monotonic, 3 keeps/3 game-ends for the Bo3, you/opp library 60 + hand 7 (was ~3). 10 new tests in `tests/test_replay_events_dq.py`; **338/338 green**. Spec: docs/superpowers/specs/2026-06-09-replay-viewer-m1-data-quality-design.md. PENDING: GUI smoke of the M3 board panel zone-counts (data layer verified, Qt path not). NEXT: (a) GUI smoke; or (b) retire classic replay dialog; or (c) M-future Odds Engine (now unblocked).)

Previous: 2026-06-04 — Event Finder UX overhaul on `feat/event-finder-ux` (numeric Distance/Entry sort, Time column, "When" filter, 300 mi radius, RCQ row tint, Google Maps right-click, persisted filters; 26 tests). 2026-05-25 — Replay-viewer M4 review annotation (notes/mark/export). M3 (2026-05-25): board panel. M2 (2026-05-24): viewer window. M1 (2026-05-22): data layer.
