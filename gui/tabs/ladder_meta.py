...
class LadderMetaTab(QWidget):
    ...
    def _build_ui(self):
        ...
        sc_lbl = QLabel(
            ...
            "(climbers = scales with skill)</span>" % theme.TEXT_DIM
        )
        sc_lbl.setStyleSheet("font-size: 11px;")
        sc_v.addWidget(sc_lbl)
        self._skill_tbl = QTableWidget(0, 6)
        self._skill_tbl.setHorizontalHeaderLabels(
            ["Archetype", "Bronze", "Silver", "Gold", "Plat", "Δ Br→Plat"]
        )
    ...
    def _fill_skill(self, rows: list, fmt: str):
        ...
        for ri, r in enumerate(rows):
            ...
            if d is not None:
                # Strong positive = green, strong negative = red
                if d >= 5:
