...
- **`search_strategy_docs(query)` — CODE-COMPLETE 2026-06-19 (`feat/strategy-doc-search`), LIVE-UNVERIFIED.** Semantic search over `../mtg-sim/docs/` (17 docs → 284 chunks) via Pinecone integrated inference. Built offline TDD: `mcp_server/strategy_search.py` (chunking + shaping), `pinecone_index.py` (lazy adapter), `config.py` ([pinecone] in config.ini), `scripts/ingest_strategy_docs.py`, `@mcp.tool` wrapper w/ graceful `index_unavailable`. 382 tests green. **BLOCKED on a Pinecone API key** — the connectivity spike + live ingest/query acceptance gate (Task 0 + Task 7) never ran, so the real Pinecone SDK calls are unexercised over the network. (2026-06-19 mitigation: validated SDK method names/signatures by introspecting installed `pinecone 9.1.0` — caught + fixed a real `upsert_records` positional-arg-order bug; calls now keyword-only. The `search(query={inputs,top_k,filter})` form is documented but still live-unconfirmed.) **TO FINISH (anyone with a key):** add key to `config.ini [pinecone]` → `python scripts/ingest_strategy_docs.py` → `pytest tests/test_strategy_search_live.py` (expects "double strike delirium prowess" → Izzet Prowess audit). Plan: `docs/superpowers/plans/2026-06-19-strategy-doc-search.md`.
...
### 5/22 session (shipped, end of day)
...
- **M1 data-quality fix** — game_num oscillation **SHIPPED 2026-06-16** (`feat/replay-match-scoping`, commit d8963c4). Real root cause was bigger than documented: `build_event_stream` never scoped to `arena_match_id` (only seat extraction used it), so every replay contained all matches from BOTH rotation logs mashed together — two different matchIds yielded byte-identical 6138-event output. Fix gates event processing to the target match's start/end region (+ `target_done` skips a duplicate copy in the other log); `SCHEMA_VERSION` 1→2 + a stale-schema load check auto-rebuilds the 18 pre-scoping whole-log caches. `build_transcript` (classic) already scoped correctly — no parallel fix. **Zone-tracking half SHIPPED 2026-06-18** (`feat/replay-zone-tracking`): root cause was that `build_event_stream` treated every `GameStateMessage`'s `zones[]` as a full snapshot, but MTGA sends mostly partial `GameStateType_Diff` messages (1395:5 in a real log) — so any zone a diff omitted got its instances evicted (~50k/match). New pure `reconcile_zones()` reconciles **per `zoneId`, only for present zones** (membership-then-evict → draw reads as a MOVE); hidden-zone cards are attributed to the zone `ownerSeatId` so **both seats'** Hand/Lib/GY/Exile counts are accurate. `SCHEMA_VERSION` 2→3 + `zone_counts` cap auto-rebuilds caches; board panel now shows all zone counts. 11 tests; 364 green. Plan: `docs/superpowers/plans/2026-06-18-replay-zone-tracking-fix.md`. **Still deferred** (genuinely absent from the `events[]`/`board_diff` contract): tap-state, +1/+1 counters, auras, combat highlighting — an extractor extension for the M-future Odds Engine.
